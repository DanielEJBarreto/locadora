...
public class VehicleService {
    ...
    public void delete(Long id) {
        ...
        if (vehicle.getStatus() != VehicleStatus.AVAILABLE) {
            throw new RuntimeException("Veículo alugado ou em manutenção não pode ser excluído");
        }
    ...
    public VehicleResponse updateStatus(Long id, VehicleStatus newStatus) {
        ...
        if (newStatus == VehicleStatus.RENTED && vehicle.getStatus() != VehicleStatus.AVAILABLE) {
            throw new RuntimeException("Veículo só pode ser alugado se estiver disponível");
        }
